...
import { controllerContent, interfaceContent, migrationContent, modelContent, routerContent } from "@utils/migrationsContent";
import fs from "fs";
...
if (flag)
    try {
        color(["🟢 Complication is complete! Start to creating files\n", "green", "bold"]);
        for (let migrate of pathName) {
